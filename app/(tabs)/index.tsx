...
function MealsSection({ date }: { date: string }) {
  ...
  const addMut = useMutation({
    mutationFn: (d: any) => {
      ...
      console.log("🚨 POST FOOD PAYLOAD START");

      console.log(JSON.stringify(d, null, 2));

      console.log("🚨 POST FOOD PAYLOAD END");

      return apiFetch("/food", { method: "POST", body: d });
...
//  console.log("Food entries:", entries);
  ...
  const grouped = MEAL_TYPES.map(mt => ({
    type: mt, items: entries.filter(e => (e.meal ?? e.mealType)=== mt),
          ...
          <ScrollView contentContainerStyle={{ padding: 16 }}>
            ...
            {results.map(item => (
              <TouchableOpacity key={item.id || item.name}
                ...
                console.log("[database food payload]", JSON.stringify(payload, null, 2));
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                console.log("[manual food payload]", JSON.stringify(payload, null, 2));
