...
Standard shadcn `Dialog` (`sm:max-w-[425px]` default, per §8). Title switches **"Set Up Fight Camp Plan"** (create) / **"Edit Fight Camp Plan"** (edit), both `font-display`. Description line: "Plan a gradual weight cut targeting 0.5–1.0% bodyweight per week." (create) or "Update your target or timeline — the plan recalculates automatically." (edit).

**Screenshot — advanced options open with manual temp reduction field visible:**
![Fight camp dialog with advanced options expanded](screenshots/fightcamp-dialog-advanced-open.jpg)

**Screenshot — manual temp reduction filled with value (2.5 kg):**
![Fight camp dialog with manual temp reduction filled](screenshots/fightcamp-dialog-manual-filled.jpg)

##### Fields (in order)

| # | Label | Input type | `data-testid` | Notes |
|---|---|---|---|---|
| 1 | Current Weight (kg) | `number step="0.1"` | `input-cut-current-weight` | Required |
| 2 | Fight Weight (kg) | `number step="0.1"` | `input-cut-target-weight` | Required |
| 3 | Fight Date | `date` | `input-cut-fight-date` | Required |
| 4 | Weigh-In Timing | 2-button group | `group-weigh-in-timing` | See below |
| 5 | Advanced options toggle | text button | `button-toggle-advanced-cut` | Collapses/reveals field #6 |
| 6 | Manual temp. reduction override (kg) | `number step="0.1" min="0" max="10"` | `input-manual-temp-reduction` | Only visible when advanced expanded |
| 7 | Submit | full-width `<Button>` | `button-submit-cut-plan` | "Start Camp Plan" / "Update Plan" |

##### Weigh-In Timing group (field 4)

Two `<button type="button">` elements in a `grid grid-cols-2 gap-2`. Each has a title line (`text-sm font-medium`) and a subtitle (`text-[11px] text-muted-foreground`).

| `data-testid` | Title | Subtitle |
|---|---|---|
| `button-timing-same_day` | Same day | Weigh in on fight day |
| `button-timing-day_before` | Day before | Weigh in the evening prior |

- **Active state:** `border-primary bg-primary/10 text-foreground` + `rounded-lg border p-3 text-left transition-colors`
- **Inactive state:** `border-border/40 hover:bg-secondary/30 text-muted-foreground`
- Helper text below the group (`text-[10px] text-muted-foreground mt-2`) — changes based on selection:
...
##### Advanced options section (field 5 + 6)

Separated by `border-t border-border/30 pt-3` from the timing group. The toggle button:
...
- `data-testid="button-toggle-advanced-cut"`

When expanded (`showAdvanced === true`), a `mt-3 space-y-1.5` div renders:
...
- This is a **free-form decimal number input — NOT a pill/chip/preset selector**. The user types any kg value (e.g. `2.5`, `3`, `1.8`). There are no preset buttons.

**Hint text (`text-[10px] text-muted-foreground`):**
> "Overrides the automatic water-weight estimate (2–6% BW) with your own value. Leave blank to keep automatic calculation. This app does not provide acute weight-cut guidance."

**Edit mode auto-expand:** When opening the edit dialog (`openEdit()`), if the existing plan has `manualTempReductionKg > 0`, `showAdvanced` is pre-set to `true` so the field is immediately visible with the saved value already populated. Closing and reopening the dialog resets `showAdvanced` to `false` unless the value was previously set.

##### Submit button

- Disabled until `currentWeight`, `targetWeight`, and `fightDate` are all non-empty strings, or while `createMutation.isPending`
- Full-width `<Button className="w-full">`
- Label: **"Start Camp Plan"** (create) / **"Update Plan"** (edit)

##### Footer disclaimer

`text-[10px] text-muted-foreground text-center` — "Educational tool only — not medical or nutritional advice."
...
##### E. Manual Temp Reduction — complete reference

This is the full authoritative specification for the `manualTempReductionKg` feature (the "Manual temp. reduction override (kg)" input in §18.1.6).

**What it is:** An optional, user-supplied estimate of how much body weight (in kg) the athlete expects to shed through temporary water/glycogen manipulation in the acute pre-weigh-in window. When provided it replaces the built-in automatic formula (6% BW for `day_before`, 1% BW for `same_day`).

**Effect on the plan — the mechanism:**

The `manualTempReductionKg` value is used to compute the **pre-final target weight** — the body weight the athlete needs to reach *before* the acute water-cut window begins. A higher manual value means the engine assumes a larger acute cut is coming, so the athlete needs to reach a higher pre-final weight, which means less total fat loss is needed during the camp phase:
...
fatLossRequired = currentWeight − preFinalTarget
                                ↑ increases if preFinalTarget is higher
weeksForFatLoss = max(0.5, (daysUntil − 7) / 7)

── same_day flow ────────────────────────────────────────────────
preFinalTarget (auto)   = targetWeight / (1 − 0.01)   ≈ +1.0% buffer
preFinalTarget (manual) = targetWeight + cappedManualKg

weeksForFatLoss = max(0.5, (daysUntil − 4) / 7)
...
**Net effect:** A larger manual temp → higher `preFinalTarget` → smaller `fatLossRequired` → lower required weekly rate → smaller daily calorie deficit during camp.

**Safety guardrails — three layers:**

| Layer | Where enforced | Constraint |
|---|---|---|
| UI input cap | `WeightCutHero.tsx` dialog, `input-manual-temp-reduction` | `max="10"` — rejects values above 10 kg at the HTML level |
| API validation | `POST /api/me/weight-cut`, `PUT /api/me/weight-cut` | `z.number().min(0).max(30)` — rejects negative values and anything above 30 kg |
| Engine cap | `shared/weight-cut.ts` inside `dayBeforeFlow()` / `sameDayFlow()` | Silently caps the value before use (see below) |

**Engine cap formula (the tightest constraint):**
...
The 10%/2% caps reflect physiological safety limits: `day_before` athletes have ~24–36h to rehydrate so a larger acute cut is survivable; `same_day` athletes compete immediately so almost no water manipulation is safe. The engine silently uses the capped value — no error is thrown and the UI shows no warning — but the `tempCut` field in the API response will reflect the capped value, not the raw user input.

**`tempCutDisplayed` — when it becomes visible:**

The `tempCutDisplayed` value in the API response is only non-zero within the final window leading up to fight day:
- `day_before`: `daysUntil ≤ 10`
- `same_day`: `daysUntil ≤ 5`

This is what drives the "Temporary reduction assumed (~Xkg)" row in the fat-loss breakdown dialog (§18.1.5). It intentionally does not appear early in camp to avoid giving the impression that water manipulation is part of the everyday plan.

**Onboarding parity:**

The same `input-manual-temp-reduction` field exists in onboarding step 22 (the Fight Camp Planner demo — see §18.9 in the Mobile Guide). The key difference is that in step 22 the field is **always visible** (not behind an Advanced options toggle). The underlying calculation is identical — `calculateWeightCutPlan()` is called with `manualTempReductionKg` in both places.

**Disclaimer wording (exact string):**
> "This app does not provide acute weight-cut guidance."

This disclaimer appears inside the hint text beneath the input in both the WeightCutHero dialog and in onboarding step 22.
...
## §24 Training Block Planner

> **Route:** `/training` — modal opened from the page header
> **File:** `client/src/components/TrainingBlockModal.tsx`, `client/src/pages/training.tsx`

### 24.0 Screenshots

**Training page — header with Plan Block button and fight-night countdown chip:**
![Training page](screenshots/training-page.jpg)

**Step 1 — Setup (block name, start date, duration selector):**
![Training Block modal Step 1](screenshots/block-modal-step1.jpg)

**Step 2 — Activities (7-day editor, each day shows Rest / + Session / + Lift):**
![Training Block modal Step 2 empty](screenshots/block-modal-step2.jpg)

**Step 2 — Add Session form open under Monday (activity search + time slot + duration + RPE):**
![Training Block modal Step 2 cardio form](screenshots/block-modal-step2-cardio.jpg)

**Step 2 — Activity search dropdown showing "boxing" results with MET values:**
![Training Block modal Step 2 search results](screenshots/block-modal-step2-search.jpg)

**Step 2 — Filled state with activities on Mon (Boxing bag work · R8 · AM) and Wed (Full Body Lifting · R7 · AM):**
![Training Block modal Step 2 filled](screenshots/block-modal-step2-filled.jpg)

**Step 3 — Load Preview: Projected Peak ACWR bar chart by week, week breakdown rows, load warnings:**
![Training Block modal Step 3 load preview](screenshots/block-modal-step3-preview.jpg)

### 24.1 Entry Point on the Training Page

Located in the training page header row (right side), always visible to authenticated users:

```
┌──────────────────────────────────────────────────────────────────┐
│  Training          [● Pre-camp Strength Block]  [📅 Edit Block]  │
│  (or, when no block active:)                    [📅 Plan Block]  │
└──────────────────────────────────────────────────────────────────┘
```

**Active block chip** (`chip-active-block`):
- `bg-primary/10 border border-primary/25 text-primary`
- `rounded-full px-3 py-1.5 text-xs font-medium`
- Calendar icon + block name text
- Only rendered when `activeBlock` is non-null

**Plan/Edit button** (`button-plan-block`):
- `variant="outline" size="sm" h-8 text-xs`
- `border-primary/30 text-primary hover:bg-primary/10`
- Calendar icon + "Plan Block" or "Edit Block" (depending on whether a block is active)

**Planned load chip** on the date card — once a block is active, the date card shows a coloured pill:

| Load | Background | Border | Text |
|---|---|---|---|
| very_hard | `bg-red-500/15` | `border-red-500/30` | `text-red-400` |
| hard | `bg-orange-500/15` | `border-orange-500/30` | `text-orange-400` |
| moderate | `bg-yellow-500/15` | `border-yellow-500/30` | `text-yellow-400` |
| light | `bg-green-500/15` | `border-green-500/30` | `text-green-400` |
| rest | `bg-muted/30` | `border-border/30` | `text-muted-foreground` |

testid: `chip-planned-load` (non-rest) or `chip-planned-rest` (rest)

---

### 24.2 Modal Container & Header

```
Dialog  max-w-lg w-full max-h-[90vh] overflow-y-auto p-0
  ┌─────────────────────────────────────────────────────┐
  │ 📅 Plan Training Block                          [X] │
  │ Define your weekly activities — sessions are        │
  │ created for each week automatically.                │
  │                                                     │
...
- Active: `bg-primary text-primary-foreground` filled circle
- Done: `bg-green-500/20 text-green-400 border border-green-500/30` with "✓"
- Future: `bg-muted text-muted-foreground` with step number
...
Block Name
  [input: "e.g. Pre-camp Strength Block"]
  testid: input-block-name

Start Date
  [input type=date]
  testid: input-block-start-date
  Note: "Defaults to next Monday."

Duration
  [ 2w ] [ 3w ] [ 4w ] [ 6w ] [ 8w ]
  testid: button-week-{n}
  Selected: bg-primary text-primary-foreground border-primary
  Unselected: border-border/40 text-muted-foreground

                          [Next: Activities →]
                          testid: button-next-schedule
                          Disabled until name+date filled
```

---

### 24.4 Step 2 — Activity Editor (7 Day Editors)

One `DayEditor` per day of week (Mon–Sun), stacked vertically with `space-y-2.5`.

**Each day card:**
```
┌─────────────────────────────────────────────────────────────────┐
│ Mon  [Light ▸]                  [🌙 Rest]  [+ Session]  [+ Lift]│
│                                                                  │
│ [⚔ Wrestling 60m · R8 AM ×]  [🏋 Full Body 45m · R7 PM ×]     │
│                                                                  │
│ (when addType = "cardio")                                        │
│ ┌──────────────────────────────────────────┐                    │
│ │ 🔵 Add Session                           │                    │
│ │ Activity: [🔍 Search session type…]       │                    │
│ │ Time: [AM] [PM] [Eve] [Night]            │                    │
│ │ Duration: [45]    Effort RPE: [7]        │                    │
│ │                      [Cancel] [Add]      │                    │
│ └──────────────────────────────────────────┘                    │
└─────────────────────────────────────────────────────────────────┘
```

**Activity chips** (inline, after adding):
- Cardio: `bg-blue-500/10 border-blue-500/25 text-blue-300` — ⚔ icon
- Lifting: `bg-purple-500/10 border-purple-500/25 text-purple-300` — 🏋 icon
- Shows: name · duration · RPE · slot abbreviation · × remove button

**Load label chip** colours (derived from AU sum):
- Rest (AU=0): muted
- Light (AU<300): green
- Moderate (AU<600): yellow
- Hard (AU<900): orange
- Very Hard (AU≥900): red

**Rest day state:** card dims (`border-border/15 bg-muted/10`), all activity chips hidden, "🌙 Rest Day" chip replaces load label.

**ActivityAddForm — Cardio:**
```
Activity  [🔍 Search input]          testid: input-activity-search
          └ Dropdown results (up to 8): name + MET value
Time      [AM] [PM] [Eve] [Night]    testid: button-slot-{slot}
Duration  [num input]                testid: input-activity-duration
RPE       [num input 1–10]           testid: input-activity-rpe
                 [Cancel]  [Add]     testid: button-add-activity
```

**ActivityAddForm — Lifting:**
```
Body Region  [Upper Body] [Lower Body] [Full Body]  testid: button-region-{value}
             Selected: bg-purple-500/20 border-purple-500/40 text-purple-300
Label        [optional input]                       testid: input-lifting-name
Time         [AM] [PM] [Eve] [Night]
Duration     [num]  RPE [num]
                       [Cancel]  [Add]
```
...
```
[← Back]                              [🔥 Preview Load]
                                      testid: button-preview-load
                                      Disabled if no activities on any day
                                      Shows "Analysing…" during loading
```

---

### 24.5 Step 3 — Load Preview

```
Projected Peak ACWR by Week
┌────────────────────────────────────────────────────────────┐
│                                              ┤ Spike       │
│                                        ─────┤ Elevated    │
│       ██                                                   │
...
│  W1   W2   W3   W4                                         │
└────────────────────────────────────────────────────────────┘
Green ≤ 1.3 · Orange 1.3–1.5 · Red > 1.5 (overtraining risk)

Per-week breakdown rows:
┌──────────────────────────────────────────┬──────────────┐
│ Week 1  1 Jul – 7 Jul                    │    0.82      │
│ [Mon] [Wed] [Fri]                        │    ACWR      │
├──────────────────────────────────────────┼──────────────┤
│ Week 2  8 Jul – 14 Jul          (orange) │    1.34      │
│ [Mon] [Tue] [Wed] [Thu] [Fri]           │    ACWR      │
│ ℹ Elevated ACWR on Mon (1.34)           │  [Cluster]   │
└──────────────────────────────────────────┴──────────────┘
```

**Warning banners:**

Red (ACWR > 1.5 — overtraining spike):
```
┌─────────────────────────────────────────────────────────┐
│ 🛡 Overtraining Risk Detected                           │
│ One or more weeks project a training spike (ACWR > 1.5) │
│ relative to your baseline. Consider adding rest days…   │
│                                                         │
│ [    I understand the risk — proceed anyway    ]        │
│              testid: button-acknowledge-overtraining     │
│              border-red-500/40 text-red-400             │
└─────────────────────────────────────────────────────────┘
```

Amber (cluster/elevated, no full spike):
```
┌─────────────────────────────────────────────────────────┐
│ ⚠ Load Warnings Detected                               │
│ Your plan has elevated load patterns…                   │
│ • Elevated ACWR on Mon (1.34)                          │
│ • High-load cluster (2+ hard days in 3)                │
│                                                         │
│ [        I've reviewed the warnings — proceed       ]   │
│              testid: button-acknowledge-warnings         │
...
```
┌──────────────────────────────────────────────────────────┐
│ ✓ Load looks sustainable — no projected spikes above 1.5 │
│   bg-green-500/8 border-green-500/20 text-green-400/80   │
└──────────────────────────────────────────────────────────┘
```

After acknowledge (orange compact notice replaces banner):
```
⚠ Overtraining risk acknowledged. Monitor your recovery closely.
```

**Step 3 footer:**
```
[← Edit Activities]                   [Commit Block]
                                      testid: button-commit-block
                                      Disabled until acknowledged (if warnings)
                                      Shows "Saving…" while mutation pending
```

---

### 24.6 Full `data-testid` Reference

| testid | Element | Step |
|---|---|---|
| `button-plan-block` | Training page Plan/Edit button | (page header) |
| `chip-active-block` | Active block name chip | (page header) |
| `chip-planned-load` | Planned load colour pill on date card | (date card) |
| `chip-planned-rest` | Planned rest pill on date card | (date card) |
| `input-block-name` | Block name text input | 1 |
| `input-block-start-date` | Start date input | 1 |
| `button-week-{2|3|4|6|8}` | Duration selector buttons | 1 |
| `button-next-schedule` | "Next: Activities" button | 1 |
| `button-rest-toggle-{0..6}` | Per-day Rest toggle | 2 |
| `button-add-cardio-{0..6}` | Per-day "+ Session" | 2 |
| `button-add-lifting-{0..6}` | Per-day "+ Lift" | 2 |
| `input-activity-search` | Cardio activity search | 2 |
| `option-activity-{id}` | Autocomplete result item | 2 |
| `button-slot-{morning|afternoon|evening|night}` | Time slot selector | 2 |
| `input-activity-duration` | Duration number input | 2 |
| `input-activity-rpe` | RPE number input | 2 |
| `button-region-{upper|lower|full}` | Lifting body region | 2 |
| `input-lifting-name` | Optional lift label | 2 |
| `button-add-activity` | "Add" in activity form | 2 |
| `chip-activity-{dow}-{i}` | Rendered activity chip | 2 |
| `button-remove-activity-{dow}-{i}` | × on activity chip | 2 |
| `button-preview-load` | "Preview Load" button | 2 |
| `button-acknowledge-overtraining` | Red risk acknowledge | 3 |
| `button-acknowledge-warnings` | Amber warnings acknowledge | 3 |
| `button-commit-block` | "Commit Block" submit | 3 |

---

### 24.7 API Calls Summary

| Call | When | Purpose |
|---|---|---|
| `GET /api/training-blocks/active` | Page load | Fetch current block + days |
| `POST /api/training-blocks/preview` | Step 2 → 3 | ACWR projection (no DB write) |
| `POST /api/training-blocks` | Commit Block | Create block + generate sessions |
| `DELETE /api/training-blocks/:id` | (from edit flow) | Archive old block |
| `GET /api/activities?query=` | Activity search (Step 2) | Autocomplete from catalog |

---

## §25 Recalculate Goals

> **Route:** `/profile` → button navigates to `/onboarding?recalculate`
> **File:** `client/src/pages/profile.tsx`, `client/src/pages/onboarding.tsx`

### 25.0 Screenshots

**Profile page — top: username, Sport Identity badge (Amateur Boxer + Fight Camp chips), Current Metrics card, Sport Identity card:**
![Profile page top](screenshots/profile-page-top.jpg)

**Profile page — bottom: Sport Identity card, Nutrition Targets card (calories / protein / carbs / fat), Push Notifications card, Recalculate Goals button:**
![Profile page bottom with Recalculate Goals button](screenshots/profile-recalculate-goals.jpg)

### 25.1 Entry Point on the Profile Page

Located at the very bottom of the Profile page, right-aligned, outside all cards:

```
Profile page (scrolled to bottom)
                                                    ┌──────────────────────┐
                                                    │ ⚙ Recalculate Goals  │
                                                    └──────────────────────┘
  variant="outline"  Settings icon  h-4 w-4 mr-2
  onClick: setLocation("/onboarding?recalculate")
```

No confirmation dialog — navigates immediately on click.

---

### 25.2 What Recalculate Mode Does

Navigating to `/onboarding?recalculate` activates a shortened version of the onboarding flow:

| Normal onboarding | Recalculate mode |
|---|---|
| Starts at splash screen (step 0) | Starts at step 3 (no splash) |
| Survey questions (~12 steps) | Skipped entirely |
| Sport selection step | Skipped (sport badge preserved) |
| Fight camp steps | Included |
| Completes to `/onboarding` welcome steps | Redirects to `/dashboard` after submit |
| All fields blank | Pre-seeded from current user profile |
...
```
[Start]  Step 3 (age/gender basics)
          │
          ↓  Steps 4–9 (height, weight, activity level, goal, experience)
          │
          ↓  Jump → Step 22 (fight camp / goal selection)
          │
          ↓  Jump → Step 28 (review + submit)
          │
          ↓  Step 29 (loading spinner — mutation in progress)
          │
          ↓  Step 30 → redirect to /dashboard
```

Steps 10–21 and 23–27 (survey, sport, projected cut chart) are all skipped.

---

### 25.4 Pre-seeding Behaviour

When the onboarding page detects `?recalculate`, it reads the user's current profile and fills:
...
| `activityLevel` | `user.activityLevel` (default `"moderate"`) |
| `goal` | `user.goal` (default `"fat_loss"`) |
| `experienceLevel` | `user.experienceLevel` (default `"beginner"`) |

The user sees their existing values already selected/filled in each step and can update any of them.

---

### 25.5 What Gets Updated After Submit

On successful `POST /api/user/:username/onboard`:
- `age`, `gender`, `height`, `currentWeight`, `activityLevel`, `goal`, `experienceLevel` overwritten
- `targetCalories`, `targetProtein`, `targetCarbs`, `targetFat` **recalculated** (Mifflin-St Jeor BMR → TDEE → goal modifier)
- Fight camp plan: updated or cleared via `POST/DELETE /api/me/weight-cut`
- Body fat %: updated via `PATCH /api/me/body-composition` (if user changed it)
- `mainSport`: **not changed** (sport selection step skipped → omitted from payload)

---

### 25.6 Visual Appearance During Recalculate

The onboarding page renders identically to normal onboarding for steps 3–28 — same card styles, progress bar, back/next buttons. Differences:

- Progress bar starts at ~9% (step 3 of ~33) rather than 0%
- No splash hero image
- Back button visible from step 4 (floor = step 3) instead of step 2
- On completion: no animated success screen — just instant redirect to dashboard

---

### 25.7 UX Pattern Notes

- The "Recalculate Goals" button is the only way to update core profile fields (age, weight, height, activity level) from the UI — there is no inline edit for these on the profile page itself.
- Fight camp targets (weight, fight date, weigh-in timing) can only be set/updated via this flow or via the WeightCutPlanner dialog on the dashboard.
- After recalculate completes, the profile page will show updated nutrition targets on next load.

---

## §26 Food Search (Search Tab) — OFF Architecture & Resilience

> This section documents the Search tab of the "Add Food" dialog. The full dialog (all 4 tabs, state, submit flow) is in the Mobile Replication Guide §9.18. This section focuses exclusively on the Search tab's OFF integration, resilience architecture, and UI states — the parts that need careful replication to avoid a broken experience during OFF outages.

### 26.1 What `/api/foods/search` does

`GET /api/foods/search?q=` is a server-side proxy that shields the client from all OFF complexity. It:
1. Checks an in-process server cache (5-min TTL, 200-entry cap)
2. Queries Off Food Facts using `fetchOFF()` with a two-endpoint fallback per region
3. Merges, deduplicates, ranks, and returns up to 20 results
4. Returns `{ results: any[], serviceUnavailable: boolean }` — always this shape

The client never calls OFF directly.

### 26.2 The two-endpoint fallback (`fetchOFF()`)

Each region query tries two OFF hosts in sequence, returning `{ items, ok }` where `ok: false` only when **both** fail:

| Attempt | URL pattern | Notes |
|---|---|---|
| Primary | `https://search.openfoodfacts.org/search?q=…` | OFF v2 Elasticsearch — faster, better relevance |
| Fallback | `https://world.openfoodfacts.org/cgi/search.pl?search_terms=…` | Legacy CGI endpoint — different server, independent infra |

`serviceUnavailable: true` is only returned by the API when **both regions' both endpoints** fail. This is a genuine outage signal, not "no results."

### 26.3 UK-first, world-fallback

Every search goes UK-first (country filter applied to both endpoints). World results (no country filter) are only fetched if UK returns fewer than 5 items. UK results win over world in deduplication.

### 26.4 Client-side caching and debounce

```
Keystroke
  │
  ├─ clearTimeout (cancel pending 250ms timer)
  ├─ key = query.toLowerCase().trim()
  ├─ offCacheRef.current.has(key)?  ──yes──▶  setOffSearchResults(cached); return immediately
  └─ setTimeout(250ms)
        └─ fetch /api/foods/search?q=…
              └─ on success: offCacheRef.current.set(key, results)  (evicts oldest if > 20 entries)
```

The client-side cache (`offCacheRef`) is a `useRef<Map>` — survives re-renders but not page refresh. Cached results are served **synchronously** with no debounce and no loading spinner.

### 26.5 `serviceUnavailable` UI states — all four empty states

| State | When | Visible text | `data-testid` | Colour |
|---|---|---|---|---|
| Loading | `isSearching === true` | "Searching…" + spinner | _(Loader2 icon)_ | muted |
| **Service unavailable** | `!isSearching && results.length === 0 && offSearchUnavailable` | "Food search is temporarily unavailable" + "The food database is having issues right now — try again in a moment, or use Barcode/Custom tabs" | `text-off-search-unavailable` | `text-amber-500` title |
| No results | `!isSearching && results.length === 0 && !offSearchUnavailable` | "No results found" + "Try a different search term or use Barcode/Custom tabs" | _(none)_ | `text-muted-foreground` |
| Idle / short query | `query.length < 2` | Search icon + "Search the Open Food Facts database" + "Type at least 2 characters to search" | _(none)_ | `text-muted-foreground` |

**Critical:** the "service unavailable" and "no results" states must be visually distinct. Showing a grey "No results found" when OFF is actually down is misleading — the amber banner specifically signals an upstream issue and directs users to alternative logging paths.

### 26.6 Search result list rendering

When results are present, `data-testid="list-off-results"` wraps a list of `<button>` elements, each `data-testid="button-off-result-{idx}"`.

Each result row (full-width, `hover:bg-accent`, `border-b last:border-0`):
- **Left:** 40×40px rounded thumbnail — product `image` if available, otherwise `UtensilsCrossed` icon in a muted placeholder
- **Middle:** `text-sm font-medium` product name (truncated) + `text-[10px] text-muted-foreground` brand (if present)
- **Right:** `text-sm font-mono font-medium` calorie count + `text-[10px]` macro shorthand (`P{n} C{n} F{n}`)

Images are loaded lazily (`loading="lazy"`). Tapping any row calls `handleSelectOffFood(food)` which closes the result list, calls the match endpoint for micronutrient suggestions, and advances to the gram-entry state.

### 26.7 Fallback tabs when OFF is down

The `serviceUnavailable` banner message explicitly names "Barcode/Custom tabs" as alternatives. These tabs work without OFF:

| Tab | Data source | OFF needed? |
|---|---|---|
| Whole Food | Local `data/ingredients.json` (~700 ingredients) + Core Foods quick-add | ❌ Never |
| Barcode | Falls back to user's custom-food DB silently if OFF barcode API is down | ❌ For custom foods |
| Custom | Fully manual — no external API | ❌ Never |

### 26.8 Ranking engine (`server/lib/searchRanking.ts`) — summary

`scoreOffResult(result, parsedQuery)` = `relevanceTier × 1000 + ukQuality`. Relevance always dominates; UK quality (max 20pts) only breaks ties within the same tier.

Two query paths:
- **Branded** (e.g. "m&s milk", "tesco bagel") — detected via `UK_BRAND_ALIASES` (55+ entries, longest-key-first matching). Tier 90 for brand + all product tokens; hard cap of tier 20 for non-brand results so wrong-brand items never outrank brand matches.
- **Generic** (e.g. "eggs", "semi skimmed milk") — token coverage against name + brand. Tier 100 for exact name match; tier 75 for all tokens anywhere in the name.

UK quality signals: `source === "uk"` (+8), UK country tag in `countries_tags` (+6), has barcode (+3), has image (+2), has protein/carbs data (+1).

The ranking engine is a **pure TypeScript module with no imports** — it can be ported verbatim or called via the API. Without it, branded UK supermarket queries return irrelevant results because brand names in OFF data often don't match what users type (e.g. "M&S" vs "marks and spencer").


---

## 27. Saved Meals & Meal Copy Actions — Complete Reference

Four distinct actions appear in the **Today's Meals** section for re-using food entries across dates. All four share the same underlying model: food entry data (name, grams, macros, source metadata) is copied verbatim by the server — no recalculation occurs.

---

### 27.1 Where these controls live

The **Today's Meals** header row (line 1760 of `dashboard.tsx`) contains one persistent button:

| Control | `data-testid` | Location |
|---|---|---|
| **Saved Meals** button | `button-saved-meals` | Top of Today's Meals card, always visible |

Inside each meal section header (Breakfast / Lunch / Dinner / Snack), when that section has ≥ 1 item, two ghost-icon buttons appear right-aligned:

| Control | `data-testid` | Icon |
|---|---|---|
| **Save as template** | `button-save-meal-{mealKey}` | `<Bookmark h-3.5 w-3.5>` |
| **Copy meal** | `button-copy-meal-{mealKey}` | `<Copy h-3.5 w-3.5>` |

Both buttons are rendered unconditionally inside `renderMealSection` but hidden via `style={{ visibility: items.length > 0 ? "visible" : "hidden" }}` — they exist in the DOM even for empty sections but are invisible.

On each food entry row, a copy icon button appears:

| Control | `data-testid` | Visibility |
|---|---|---|
| **Copy food item** | `button-copy-food-{entry.id}` | `opacity-100` on mobile; `sm:opacity-0 group-hover:opacity-100` on desktop |

---

### 27.2 Feature A — Save Meal as Template

**Component:** `SaveMealDialog({ date, meal, mealLabel })` (dashboard.tsx ~line 1941)

**Trigger:** Bookmark icon on the meal section header row.

**Dialog:**
- Title: "Save {mealLabel} as Template" (`font-display`)
- Description: "Save this meal so you can quickly log it again on another day."
- Input: `id="template-name"`, `data-testid="input-template-name"`, placeholder `e.g. My {mealLabel}`
- Enter key submits
- CTA: "Save Template" → "Saving..." while pending — `data-testid="button-confirm-save-meal"`

**Hook:** `useSaveMealTemplate()` — sends `POST /api/me/meals/templates/from-log` with `{ date, meal, name }`.

**Server behaviour (`routes.ts` lines 1273–1310):**
1. Fetches all `food_entries` for the user on `date`
2. Filters to entries where `entry.meal === meal`
3. Returns 400 if 0 matching entries
4. Inserts a `meal_templates` row: `{ userId, name, defaultMeal: meal, createdAt: now().toISOString() }`
5. Inserts one `meal_template_items` row per entry — copies: `name`, `grams`, `calories`, `protein`, `carbs`, `fat`, `fibre`, `sourceType`, `ingredientIndex`, `offBarcode`, `macroSource`, `microSource`, `sortOrder` (index in array)
6. Returns `201 { ...template, items }`

**Cache invalidation on success:** `["/api/me/meals/templates"]` — the Saved Meals list refreshes.
**Toast:** "Meal Saved — Your meal has been saved as a template."

---

### 27.3 Feature B — Saved Meals Library (log a template)

**Component:** `SavedMealsDialog({ selectedDate })` (dashboard.tsx ~line 2039)

**Trigger:** "Saved Meals" outline button (`<BookOpen h-4 w-4 mr-1> Saved Meals`) — `data-testid="button-saved-meals"`.

**Dialog layout:** `max-h-[80vh] overflow-y-auto` — scrollable on long lists.

**State A — Template list view** (default):
- Loading: single `<Skeleton className="h-16 w-full">` while fetching
- Empty state: `"No saved meals yet. Use the bookmark icon next to a meal to save it."`
- Each template card (`data-testid="template-{t.id}"`):
  - Name: `font-medium text-sm`
  - Metadata row: `defaultMeal` badge (variant="secondary", text-[10px]) + `"{n} items"` + `"{total kcal} kcal"` — these are computed **client-side** from `t.items.reduce()`
  - Action buttons: `"Log"` (`button-log-template-{t.id}`, size="sm", variant="outline") + trash icon (`button-delete-template-{t.id}`, size="icon", variant="ghost")

**Clicking "Log"** transitions to state B, pre-filling `logDate = selectedDate` and `logMeal = t.defaultMeal || "lunch"`.

**State B — Log confirmation view:**
- Title text: `"Log "{logTarget.name}""`
- Date picker: `data-testid="input-log-template-date"` — pre-filled with `selectedDate`
- Meal select: `data-testid="select-log-template-meal"` (breakfast / lunch / dinner / snack)
- "Back" button returns to State A
- CTA: "Log Meal" → "Logging..." — `data-testid="button-confirm-log-template"`

**Hook:** `useLogMealTemplate()` — sends `POST /api/me/meals/templates/:id/log` with `{ date, meal }`.

**Server behaviour (`routes.ts` lines 1312–1348):**
1. Validates template ownership (`template.userId === auth.profileUser.id`)
2. Returns 400 if template has 0 items
3. Calls `storage.createFoodEntries(items.map(...))` — creates one `food_entries` row per template item, using the target `date` and `meal`, and copying all macro/source fields verbatim
4. Returns `201 { count: N, entries: [...] }`

**Cache invalidation on success:** `invalidateFoodQueries(queryClient)` — food entry cache for all dates refreshes.
**Toast:** `"Meal Logged — {count} items added to your log."`

**Delete template:**
- Hook: `useDeleteMealTemplate()` — sends `DELETE /api/me/meals/templates/:id`
- Validates ownership, hard-deletes from `meal_templates` (cascade deletes items)
- Invalidates `["/api/me/meals/templates"]`
- Toast: "Template Deleted — Saved meal has been removed."

---

### 27.4 Feature C — Copy Meal (whole section)

**Component:** `CopyMealDialog({ fromDate, fromMeal, mealLabel })` (dashboard.tsx ~line 1986)

**Trigger:** Copy icon on the meal section header row — `data-testid="button-copy-meal-{fromMeal}"`.

**Dialog:**
- Title: "Copy {mealLabel}"
- Description: "Copy all items from this meal to another day."
- Target date: `id="copy-to-date"`, `data-testid="input-copy-to-date"` — starts empty
- Target meal select: `data-testid="select-copy-to-meal"` — pre-filled to `fromMeal`
- CTA disabled until `toDate` is non-empty
- CTA: "Copy Meal" → "Copying..." — `data-testid="button-confirm-copy-meal"`

**Hook:** `useCopyMeal()` — sends `POST /api/me/meals/copy` with `{ fromDate, fromMeal, toDate, toMeal }`.

**Server behaviour (`routes.ts` lines 1350–1383):**
1. Validates `fromDate`, `fromMeal`, `toDate` are present; `toMeal` defaults to `fromMeal` if absent
2. Fetches food entries for `(userId, fromDate)`, filters to `fromMeal`
3. Returns 400 if no entries found
4. Calls `storage.createFoodEntries(...)` with new `date = toDate`, `meal = targetMeal`, all other fields copied verbatim
5. Returns `201 { count: N, entries: [...] }`

**Cache invalidation on success:** `invalidateFoodQueries(queryClient)`.
**Toast:** `"Meal Copied — {count} items copied successfully."`

> **Note:** `snackIndex` is NOT copied — all new entries receive `snackIndex = null` from the create path. If you add multiple snack copies, they may all default to snack index 1 on the target date.

---

### 27.5 Feature D — Copy Food Item (single entry)

**Component:** `CopyFoodDialog({ entry })` (dashboard.tsx ~line 4787)

**Trigger:** Copy icon on individual food entry row — `data-testid="button-copy-food-{entry.id}"`.

**Dialog (max-width 360px):**
- Title: "Copy item"
- Description: "{entry.name} — choose a target date and meal."
- Date: `id="copy-food-date"`, `data-testid="input-copy-food-date"` — resets to `entry.date` each time dialog opens
- Meal select: `data-testid="select-copy-food-meal"` — resets to `entry.meal || "lunch"` on open; select items include meal icons (Coffee / Sun / Moon / Cookie)
- CTA disabled until `toDate` non-empty
- CTA: "Copy item" → "Copying..." — `data-testid="button-confirm-copy-food"`

**Hook:** `useCreateFoodEntry()` — this is the **same hook used for adding food** — no dedicated copy endpoint.

**Fields copied verbatim:**
`name`, `grams`, `calories`, `protein`, `carbs`, `fat`, `fibre`, `sourceType`, `offBarcode`, `ingredientIndex`, `macroSource`, `microSource`, `enteredBasis`, `normalizedGrams`, `yieldFactorUsed`

**Hardcoded overrides during copy:**
- `meal` = user-selected `toMeal`
- `snackIndex` = `toMeal === "snack" ? 1 : null` (always index 1, not preserved from source)
- `date` = user-selected `toDate`

**Cache invalidation on success:** `invalidateFoodQueries(queryClient)` (via `useCreateFoodEntry`).
**Toast:** `"Copied — {entry.name} copied to {toDate}."`
...
No foreign-key constraint from `meal_template_items.template_id` to `meal_templates.id` in the Drizzle schema — deletion is handled explicitly in `storage.deleteMealTemplate()`.

---

### 27.7 Server vs. client responsibility summary

| Operation | Where | Notes |
|---|---|---|
| Read template list + hydrate items | **Server** | `GET /api/me/meals/templates` — Promise.all to fetch items per template |
| Create template from log | **Server** | Reads live food entries; creates template + items atomically |
| Log template to date | **Server** | Creates food entry rows; no macro recalculation |
| Copy whole meal | **Server** | Reads food entries; creates copies; no recalculation |
| Delete template | **Server** | Hard delete; items deleted via explicit storage call |
| Copy single food item | **Server** (standard) | Uses `POST /api/me/food-entries` — same as adding food |
| Display template total kcal/item count | **Client** | `t.items.reduce()` in `SavedMealsDialog` template list |
| Button visibility (save/copy icons on meal header) | **Client** | CSS `visibility` toggle; not conditional render |
| Dialog state (log-confirm vs. list) | **Client** | `useState(logTarget)` — null = list, object = confirm view |
| Meal icon in copy-food select | **Client** | Decorative only (Coffee/Sun/Moon/Cookie) |

---

### 27.8 Screenshots

**Dashboard — Today's Meals section showing the persistent "Saved Meals" button in the meal header area (no food entries yet):**
![Dashboard overview — Saved Meals button visible](screenshots/meals-today-logged.jpg)

---
## 28. Full Auth Flow — Complete Reference

### 28.1 Overview

PRFMR uses two parallel auth systems:
- **Local auth** — email + password via Passport.js `LocalStrategy`
- **Google OAuth 2.0** — via `passport-google-oauth20`

Both share the same session store (PostgreSQL) and produce identical session cookies. A user who registers locally can later sign in with Google (email match auto-links the accounts). There is no magic-link or phone-based auth.

---

### 28.2 Database Schema

**`users_auth`** — identity layer (one row per account)

| Column | Type | Notes |
|---|---|---|
| `id` | serial PK | |
| `email` | text UNIQUE NOT NULL | always lowercased |
| `username` | text UNIQUE NOT NULL | 3–20 chars, `^[a-z0-9_]+$` |
| `password_hash` | text NULLABLE | NULL for pure Google accounts |
| `email_verified` | boolean DEFAULT false | Google accounts auto-verified |
| `email_verified_at` | timestamp NULLABLE | |
| `provider` | text DEFAULT 'local' | `'local'` or `'google'` |
| `provider_user_id` | text NULLABLE | Google profile ID |
| `source` | text DEFAULT 'unknown' | traffic source tag |
| `created_at` | timestamp | |
| `updated_at` | timestamp | |

**`users`** — profile/onboarding layer (one row per account, FK → `users_auth.id`)

| Column | Type | Notes |
|---|---|---|
| `id` | serial PK | |
| `username` | text UNIQUE NOT NULL | mirrors `users_auth.username` |
| `auth_user_id` | integer FK | references `users_auth.id` |
| `age`, `gender`, `height`, `current_weight` | various | set during onboarding |
| `activity_level`, `goal`, `experience_level` | text | |
| `target_calories/protein/carbs/fat` | integer | calculated post-onboarding |
| `main_sport` | text | e.g. "Amateur Boxer" |
| `profile_photo_url` | text | |
| `body_fat_pct` | real | 0–1 decimal (0.15 = 15%) |
...
### 28.3 Session Configuration

- **Store**: PostgreSQL via `connect-pg-simple`, table name `session` (auto-created)
- **Cookie name**: `connect.sid`
- **Flags**: `httpOnly: true`, `sameSite: 'lax'`, `secure: true` (production only)
- **Expiry**:
  - Browser sessions: **30 days**
  - PWA / standalone sessions: **90 days** (detected via `x-app-mode: standalone` request header)
- Session expiry is refreshed on every authenticated request if the current `maxAge` doesn't match the target

---

### 28.4 Password Rules

Validated both client-side and server-side (Zod):
- Minimum **8 characters**
- At least **1 letter** and **1 number**
- Regex: `/^(?=.*[a-zA-Z])(?=.*\d)/`
- Hashed with **argon2** (not bcrypt)
...
- 3–20 characters
- Only `a-z`, `0-9`, `_` (lowercase enforced server-side)
- Cannot start or end with `_`
- No consecutive `__`
- Validated by `shared/username-validation.ts`
- Rate-limited: max **3 changes per 24 hours** (tracked in `username_changes` table)

---

### 28.6 API Endpoints

All local-auth endpoints are mounted at **`/api/auth/`**.
...
```
- `inviteToken` is **required** — registration is invite-only
- `source` is optional (one of: `ig`, `tiktok`, `reddit`, `discord`, `gym`, `uni`, `friend`, `google`, `facebook`, `sideproject`, `other`, `unknown`)

Success `201`:
```json
{ "user": { "id": 42, "email": "user@example.com", "username": "username", "emailVerified": false } }
```

Error responses:
- `400` — validation error (bad email format, weak password, invalid username)
- `403` — missing invite, invalid/expired invite, or beta cap full
- `409` — email or username already taken
- `500` — server error

Side effects on success: creates row in `users_auth` + `users`, marks invite used, sends verification email + welcome email, logs user in (session created).
...
```
- `identifier` accepts **email OR username** (case-insensitive, trimmed)
- Accounts with no `password_hash` (Google-only) return `401` with: `"This account uses Google login. Please sign in with Google."`

Success `200`:
```json
{ "user": { "id": 42, "email": "user@example.com", "username": "username", "emailVerified": true } }
```
...
No body required. Destroys session + clears `connect.sid` cookie.
...
Use this as your **session check** on app startup.
...
Always returns `200` with generic message (email enumeration protection):
```json
{ "message": "If that email is registered, you will receive a password reset link." }
```

Side effect: sends password reset email with a token link.
...
Token is single-use and invalidated on success.

---

#### `POST /api/auth/resend-verification`
Rate-limited: 3 req / 15 min per IP. Requires active session.
...
Processes the email verification token from the link in the verification email.

- On success: **302 redirect** → `/verify-email?success=1`
- On failure: **302 redirect** → `/verify-email?error=<reason>` (reasons: `invalid`, `expired`, `server`)

---

#### `POST /api/auth/change-email`
Requires active session. Rate-limited: 10 req / 15 min.
...
Sets `email_verified = false`, sends new verification email.
...
#### `GET /api/auth/username-available?username=<username>`
No auth required. Returns availability + format validation.
...
Requires active session. Rate-limited: 3 changes per 24 hours.
...
- `VALIDATION_ERROR` — format invalid
- `INVALID_FORMAT` — username validation failed
- `SAME_USERNAME` — identical to current
- `USERNAME_TAKEN` — already in use
- `RATE_LIMITED` — too many changes in 24h (includes `nextAllowedAt` ISO timestamp)

---

### 28.7 Google OAuth Flow

#### Initiation
```
GET /auth/google?src=<optional-source>
```
- `src` param (optional) is the traffic source — saved in session for post-callback attribution
- Redirects to Google consent screen with scopes: `profile`, `email`
- Requires `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` env vars (flow disabled if missing)

#### Callback
```
GET /auth/google/callback
```
On success: **302 redirect** → `/dashboard`
On failure: **302 redirect** → `/?error=google_auth_failed`

#### What the strategy does server-side:
1. Receives Google profile (email + Google user ID)
2. Looks up `users_auth` by email
3. **If found (existing Google user):** returns existing user
4. **If found (existing local user, unverified email):** blocks with error — "verify your email first"
5. **If found (existing local user, verified email):** links Google to local account (`provider → 'google'`, `email_verified → true`), returns user
6. **If not found (new user):** creates `users_auth` + `users` rows, auto-generates username from `displayName` (sanitized, collision-safe with numeric suffix), marks `email_verified: true`

#### Callback URL for Google Cloud Console:
```
https://<your-production-domain>/auth/google/callback
```
Set `GOOGLE_CALLBACK_URL` env var if the auto-detected domain is wrong.

---

### 28.8 Auth Flow Diagrams

**Local login flow:**
```
Client → POST /api/auth/login { identifier, password }
  Server: normalize identifier → look up users_auth by email OR username
  Server: verify argon2 hash → create session → set connect.sid cookie
Client ← 200 { user: { id, email, username, emailVerified } }
Client: stores username in localStorage ("fitness_app_username")
Client: invalidates /api/user/me cache → redirects to /dashboard or /onboarding
```

**Google OAuth flow (mobile WebView / browser):**
```
Client → GET /auth/google?src=<source>        (browser redirect, not XHR)
  Server: saves src in session → passport.authenticate('google') → redirect to Google
Google → user consents → redirect to /auth/google/callback?code=...
  Server: exchanges code → gets profile → upserts users_auth + users
  Server: req.login() → session created → connect.sid cookie set
Client ← 302 redirect to /dashboard
Client: GET /api/auth/me → confirms session active
```

**Registration flow:**
```
Client → POST /api/auth/register { email, username, password, inviteToken, source }
  Server: validate invite token → check beta cap → hash password
  Server: INSERT users_auth + INSERT users → mark invite used
  Server: send verification email + welcome email (async, non-blocking)
  Server: req.login() → session created
Client ← 201 { user: { id, email, username, emailVerified: false } }
Client → /onboarding (redirected automatically)
```

---

### 28.9 Client Auth State Management

All auth state lives in `client/src/hooks/use-auth.ts`:

- `useAuth()` — React Query query on `GET /api/auth/me`, cached 5 min, returns `AuthUser | null`
- `useLogin()` — mutation → `POST /api/auth/login`
- `useRegister()` — mutation → `POST /api/auth/register`
- `useLogout()` — mutation → `POST /api/auth/logout`, clears query cache + localStorage
- `useResendVerification()` — mutation → `POST /api/auth/resend-verification`
- `useChangeEmail()` — mutation → `POST /api/auth/change-email`

Username is also persisted to `localStorage` key `"fitness_app_username"` as a convenience (not used for auth decisions).

Route guard: `client/src/components/protected-route.tsx` — if `useAuth()` returns null, redirects to `/login`.

---

### 28.10 Rate Limits Summary
...
| `/api/auth/resend-verification` | 15 min | 3 |
| `/api/auth/change-email` | 15 min | 10 |
| `/api/auth/username` (PATCH) | 24 hours | 3 changes |

---

### 28.11 Screenshots

**Login page** (`/login`) — Google button + email/password form:
![Login page](screenshots/auth-login-full.jpg)

**Register / Invite Request page** (`/register`) — Google button + invite request form:
![Register page](screenshots/auth-register-full.jpg)

**Forgot password page** (`/forgot-password`):
![Forgot password page](screenshots/auth-forgot-password-full.jpg)

**Reset password page** (`/reset-password`) — token from email:
